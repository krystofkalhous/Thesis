...
class Bank:
      def __init__(self,ide,homecountry,A,Lcountry,Fcost,folder,name,run,delta,minReserve\
          ...
          self.bank_spatial_position = spatial_position # spatial position drawn from U[0, 1) that is later used in the Hoteling circle
          self.screening_cost_big = 0.01 # magical constant, screening costs of big banks
          ### ----------------------------------------------------------------
      ...
      def computeInterestRate(self,leverage, loan_amount, bank_spatial_position, firm_spatial_position): ### added
          ...
          if self.bank_type == "bank_Big" and loan_amount > 0:
              interestRate += self.screening_cost_big / max(loan_amount, 1e-9) #big banks add screening costs to interest rate

          if self.bank_type == "bank_Small":
              distance = self.circular_distance(position_Firm = firm_spatial_position, position_Bank = bank_spatial_position)
              small_bank_max_spatial_distance = 0.015 # before 0.01 # before 0.005
              small_bank_rate = 2 # try 2-6? #decay steepness
              small_rate_max = 0.01 # experiment # discount rate by small banks
   
              kernel = math.exp(- small_bank_rate * distance / small_bank_max_spatial_distance)

              discount = small_rate_max * kernel
      ...
      def computeProbProvidingLoan(self,leverage,relPhi, bank_spatial_position, firm_spatial_position, demanded_loan):
          
          ### Added: bank_Big incurs screening costs to observe RelPhi, i.e. productivity of the firm.
          if self.bank_type == 'bank_Big': 
              if demanded_loan <= self.screening_cost_big:
                  probLoan = 0.0
                  return probLoan
          ### 
          ...
          if self.bank_type == 'bank_Small':
              spatial_distance = self.circular_distance(position_Firm = firm_spatial_position, position_Bank = bank_spatial_position)
              if spatial_distance <= 0.005: ### modify (originally 0.10)
                  probLoan=math.exp(-1*self.iota*leverage)
                  if relPhi<=1.0:
                      probLoan=math.exp(-1*self.iotaRelPhi*leverage)
                  return probLoan
                  
              if spatial_distance > 0.005:
                  probLoan = 0.0
                  return probLoan
