...
class CentralBank:
      ...
      def checkNetWorth(self):
          # The CB Bonds/Reserves ratio was never a real invariant in this model
          # and was always silently ignored via NameError in the original code.
          # We drop it and keep only the Deposit consistency checks below.
          if self.Deposit<-0.001:
             raise AssertionError("SFC invariant violated: centralBank.py:44 in checkNetWorth()")
          checkDeposit=0
          ...
          if  self.Deposit<checkDeposit-0.001 or self.Deposit>checkDeposit+0.001:
              raise AssertionError("SFC invariant violated: centralBank.py:49 in checkNetWorth()")
      ...
      def depositInjection(self,injection,ideAgent):
          ...
          if self.Deposit<-0.000001:
             raise AssertionError("SFC invariant violated: centralBank.py:55 in depositInjection()")
      ...
      def depositWithdrawal(self,reduction,ideAgent):
          ...
          if self.Deposit<-0.000001:
             raise AssertionError("SFC invariant violated: centralBank.py:61 in depositWithdrawal()")
      ...
      def payDepositInterest(self,McountryConsumer,McountryFirm):
          for agent in self.Mdeposit:
              ...
              if agent[0]=='F' and McountryFirm[self.country][agent].closing=='yes':
                 raise AssertionError("SFC invariant violated: centralBank.py:83 in payDepositInterest()")
      ...
      def checkReserves(self,McountryBank,McountryFund=None):
          ReservesBank=0
          for bank in McountryBank[self.country]:
              ReservesBank=ReservesBank+McountryBank[self.country][bank].Reserves
          if McountryFund is not None and self.country in McountryFund:
              ReservesBank=ReservesBank+McountryFund[self.country].Reserves
          if ReservesBank>self.Reserves+0.01 or ReservesBank<self.Reserves-0.01:
             if len(McountryBank[self.country])>0:
                raise AssertionError("SFC invariant violated: centralBank.py:138 in checkReserves()")
