...
class Bank:
      def __init__(self,ide,homecountry,A,Lcountry,Fcost,folder,name,run,delta,minReserve\
          ...
          self.bankSaving=0 
          self.lockedReserves=0.0  # cumulative retained surplus held as indivisible reserve (tally on A)
          self.lockReserves=False  # set True by enterExit for relationship banks when enabled
          self.pastProfit=0
      ...
      def computeInterestRate(self,leverage):
          ...
          if interestRate<-0.001:
             raise AssertionError("SFC invariant violated: bank.py:69 in computeInterestRate()")
          return interestRate
      ...
      def existence(self,McountryConsumer,McountryFirm,McountryCentralBank,rBonds,McountryEtat,DcountryAvWage,fund=None):
          ...
          # Relationship-fund recapitalisation: if a regional bank is about to
          # fail (equity at/below the viability floor), the fund tops it up to a
          # viable level — but ONLY if it can cover the FULL amount. Otherwise it
          # spends nothing and the bank falls through the normal exit path below
          # (and the region re-founds a bank in enter()). This runs BEFORE the
          # exit block so a successful recap keeps the bank open this period.
          if (fund is not None and getattr(fund,'on',False) and getattr(fund,'recapOn',True)
                  and getattr(self,'isRegional',False) and self.closing=='no'):
              floor=1*self.Fcost*DcountryAvWage[self.country]
              if self.A<=floor or self.A<=0.0:
                  target=fund.recapKappa*floor
                  fund.attemptRecap(self,target,McountryCentralBank)
      ...
      def distributingDividends(self,McountryConsumer,McountryBank,McountryCentralBank,
                                fund=None,gammaEff=0.0):
          self.dividending()
          # Relationship-fund contribution: a regional bank diverts a fraction
          # gammaEff of this period's retained surplus to the protection fund.
          # This must happen BEFORE capitalVariation re-marks owner stakes, so the
          # contributed surplus correctly leaves household-claimable equity. It is
          # capped at available reserves so the bank never borrows to contribute.
          self.fundContribution=0.0
          if (fund is not None and gammaEff>0.0
                  and getattr(self,'isRegional',False)
                  and self.closing=='no' and self.bankSaving>0.0):
              c=gammaEff*self.bankSaving
              if c>self.Reserves:
                  c=self.Reserves
              if c>0.0:
                  self.A=self.A-c
                  self.bankSaving=self.bankSaving-c
                  if self.lockedReserves>self.A:
                      self.lockedReserves=self.A
                  self.fundContribution=c
                  fund.receiveContribution(c,self,McountryCentralBank)
      ...
      def dividending(self):
          ...
          # Relationship banks: retained surplus accrues to the indivisible reserve.
          # self.A is deliberately unchanged - lockedReserves is only a TALLY of how much
          # of A is non-reinvestable; the money stays in A, stays marked to owners, stays
          # in every SFC identity. The behavioral lock is on the consumer side. The clamp
          # keeps 0 <= lockedReserves <= A so loss-taking/taxation (which lowered A earlier
          # in the period) draw the reserve buffer down first.
          if getattr(self,'lockReserves',False) and getattr(self,'isRegional',False):
             if self.closing=='no' and self.bankSaving>0:
                self.lockedReserves=self.lockedReserves+self.bankSaving
          if self.lockedReserves>self.A:
             self.lockedReserves=self.A
          if self.lockedReserves<0:
             self.lockedReserves=0.0
          ...
          for consumeride in self.Downer:
              if self.closing=='yes':# or self.A<=0:
                    ...
                    if  DismissShare<-0.0000001:
                        raise AssertionError("SFC invariant violated: bank.py:190 in capitalVariation()")
                    del McountryConsumer[self.homecountry][consumeride].DLA[self.ide]                    
                    if self.capitalDismiss<-0.00000001:
                       raise AssertionError("SFC invariant violated: bank.py:193 in capitalVariation()")
              else:
                  ...
                  if  McountryConsumer[self.homecountry][consumeride].DLA[self.ide][2]<-0.000001:
                      raise AssertionError("SFC invariant violated: bank.py:200 in capitalVariation()")
                  if self.Downer[consumeride][2]<McountryConsumer[self.homecountry][consumeride].DLA[self.ide][2]-0.00000001 or\
                     ...
                     raise AssertionError("SFC invariant violated: bank.py:203 in capitalVariation()")
                  ...
                  if  DismissShare<-0.00000001:
                        raise AssertionError("SFC invariant violated: bank.py:207 in capitalVariation()")
                  ...
                  McountryConsumer[self.homecountry][consumeride].receiving(DismissShare,McountryBank,McountryCentralBank)            
                  if self.capitalDismiss<-0.00000001:
                     raise AssertionError("SFC invariant violated: bank.py:219 in capitalVariation()")
          self.reserveWithdrawal(totalPayement,McountryCentralBank) 
          if  givenA<self.A-0.0001 or givenA>self.A+0.0001:
              raise AssertionError("SFC invariant violated: bank.py:222 in capitalVariation()")
 
      def reviseA(self):
          if self.A<=self.Fcost or self.A<=0.0:
             raise AssertionError("SFC invariant violated: bank.py:226 in reviseA()")
      ...
      def repayingCBLoan(self,McountryCentralBank):
          ...
          if reduction>self.Reserves+0.0000000001:
             raise AssertionError("SFC invariant violated: bank.py:266 in repayingCBLoan()")
          McountryCentralBank[self.country].Reserves=McountryCentralBank[self.country].Reserves-reduction
      ...
      def loanCreationDomestic(self,firmIde,loanValue,interestRate,countryFirm):
          if (firmIde in self.Mloan)==True:
             raise AssertionError("SFC invariant violated: bank.py:293 in loanCreationDomestic()")
          ...
          if self.Mdeposit[firmIde][3]<-0.001:
             raise AssertionError("SFC invariant violated: bank.py:302 in loanCreationDomestic()")
 
      def loanCreationForeigner(self,firmIde,loanValue,interestRate,countryFirm,McountryCentralBank):
          if (firmIde in self.Mloan)==True:
             raise AssertionError("SFC invariant violated: bank.py:306 in loanCreationForeigner()")
          self.Mloan[firmIde]=[firmIde,self.ide,loanValue,interestRate,countryFirm]
          self.Loan=self.Loan+loanValue
          reduction=loanValue
          if reduction>self.Reserves:
      ...
      def askingLoanCentralBank(self,askingLoan,McountryCentralBank):
          ...
          if self.loanDiscount<0:
             raise AssertionError("SFC invariant violated: bank.py:385 in askingLoanCentralBank()")
      ...
      def checkNetWorth(self):
          ...
          printing='no'  
          printingControl='no'
          # Skip during bootstrap: new bank has A>0 but Reserves=0 until
          # allocatingConsumerDeposit runs. Original code silently NameError'd.
          preBank = (self.A > self.Reserves + 0.001 and self.Loan < 0.001)
          if not preBank:
              if (Liabilities+Assets)>0 and ((Liabilities-Assets)/float(Liabilities+Assets)>0.0001 or (Liabilities-Assets)/float(Liabilities+Assets)<-0.0001 or self.Reserves<-0.001):
                 # DISABLED — known STRUCTURAL imprecision, tolerated by design
                 # (the original model disabled this check too). NOT float noise and
                 # NOT the firm's benign A/cash drift; it is a small CONSTANT per-bank
                 # residual (~0.31, ~0.1% of the bank, non-accumulating, non-exploding).
                 #
                 # Root cause (traced): the bank books EXPECTED loan-interest income on
                 # its loan book in Bank.existence (potentialEnter = Σ principal*rate),
                 # but interest is actually COLLECTED earlier, when firms repay
                 # (repayingLoan credits Reserves by principal+interest while Loan falls
                 # by principal only). Because firms run existence/repay BEFORE banks
                 # recognize income, expected != collected whenever a borrower exits or
                 # defaults that period — the gap is booked into A without backing
                 # reserves and then persists.
                 #
                 # This is a settlement-ORDERING issue, not a one-line bug. Fixing it
                 # correctly means re-architecting interest recognition (book interest at
                 # collection, drop the expected-interest term) and re-validating, which
                 # changes model dynamics. By decision it is left disabled and tolerated.
                 pass
