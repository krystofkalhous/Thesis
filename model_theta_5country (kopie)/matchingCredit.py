...
# --- single-country isolation: when on, a firm's transaction-bank candidates are restricted to
#     banks of its OWN country (no cross-border lending). Regional banks are already within-country.
_XBORDER_CREDIT_OFF=os.environ.get("XBORDER_CREDIT_OFF","0").lower() in ("1","true","yes","on")
# ---- theta extension (borrower self-selection on latent type); default OFF ----
_THETA_ON=os.environ.get("THETA_ON","0").lower() in ("1","true","yes","on")  # master switch
_SELF_NOISE=float(os.environ.get("SELF_NOISE","0.3"))   # sigma of firm's private self-estimate noise
_GAP_KAPPA=float(os.environ.get("GAP_KAPPA","0.5"))     # size hurdle in self-selection (bigger => big firms stay transactional)
...
class MatchingCredit:
      ...
      def matchCreditOpen(self,McountryFirm,McountryBank,McountryCentralBank):
          ...
          for drow in self.MloanDemand:
              ...
              # --- theta self-selection signal (theta extension; only when THETA_ON) ---
              # The firm holds private info on its latent ceiling type. It forms a
              # noisy self-estimate thetaTilde and perceives an upside =
              # (believed ceiling - current phi)/frontier. _phiHat is the perceived-
              # ceiling productivity the relationship lender ranks/prices on (below).
              _phiHat=firmPhi
              if _THETA_ON:
                  _relPhi0=drow[6]                                  # = phi / frontier_i (set when demand row built)
                  _Fi = (firmPhi/_relPhi0) if _relPhi0>0 else 0.0    # recover firm's relevant frontier
                  _eps = random.gauss(0.0,_SELF_NOISE) if _SELF_NOISE>0 else 0.0
                  _thetaTilde = getattr(firmObj,'thetaType',1.0)*math.exp(_eps)
                  _phiHat = _thetaTilde*_Fi
              ...
              if regBankIde is not None and regBankIde not in firmObj.Lcreditor:
                  if _THETA_ON:
                      # gap-based self-selection on PERCEIVED unrealised upside:
                      # small firms (low sizeScore) that believe their ceiling sits
                      # well above their current phi self-select into the relationship
                      # lender; mature firms (phi near ceiling) and low-type firms
                      # perceive little upside and go transactional.
                      _relPhi=drow[6]                       # = phi / frontier_i
                      _uHat=_thetaTilde-_relPhi              # perceived upside, frontier units
                      _ss=self._sizeScore(firmA,countryFirm)
                      _p=(max(0.0,min(1.0,_uHat-_GAP_KAPPA*_ss)))**_APPLY_GATE_EXP
              ...
              if _useRegHere:
                  globalCandidates=[ide for ide,srow in supplyByIde.items()
                                    ...
                                    and (not _XBORDER_CREDIT_OFF or srow[2]==countryFirm)]
              else:
                  globalCandidates=[ide for ide,srow in supplyByIde.items()
              ...
              for ideBank in approached:
                  ...
                  if isReg and phiRegion>0 and _phiHat>0:
                      relScore=_phiHat/phiRegion
                      phiR=phiRegion/_phiHat
