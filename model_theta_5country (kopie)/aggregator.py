...
class Aggregator:
      ...
      def income(self,McountryConsumer,t,run,McountryFirm,McountryEtat,McountryBank,McountryCentralBank,DglobalPhi,DcountryFirmEnter,\
          ...
          for country in McountryConsumer:
              ...
              # badDebt fix: income() runs before exitFirm() each period, so
              # bankLosses above is always 0 at this point. DpastLosses holds
              # the losses recorded by checkCA() AFTER exitFirm() last period,
              # i.e. realised bad debt with a one-period lag (row t = bad debt of t-1).
              badDebtRealized=self.DpastLosses[country]
              for consumer in  McountryConsumer[country]:
                  totLabor=totLabor+McountryConsumer[country][consumer].ls
              ...
              if lUsedC<LC-0.000001 or  lUsedC>LC+0.000001:
                 raise AssertionError("SFC invariant violated: aggregator.py:484 in income()")
              ...
              self.DcountryCollectData[country]=[run,t,country,productionC,DcountryExport[country],DcountryImport[country],nfirmC,\
                                   ...
                                   productionCSold+changeInventoryValue,McountryEtat[country].Salvatage,badDebtRealized,\
                                   ...
                                   regionalCreditData[country][0] if regionalCreditData and country in regionalCreditData else 0,\
                                   regionalCreditData[country][1] if regionalCreditData and country in regionalCreditData else 0,\
                                   regionalCreditData[country][2] if regionalCreditData and country in regionalCreditData else 0,\
                                   regionalCreditData[country][3] if regionalCreditData and country in regionalCreditData else 0]
          totalTB=0
          totalY=0      
          for country in McountryConsumer:
              TBC=DcountryExport[country]-DcountryImport[country] 
          ...
          if R<0:
             raise AssertionError("SFC invariant violated: aggregator.py:541 in income()")
      ...
      def checkCA(self,McountryConsumer,t,run,McountryFirm,McountryEtat,McountryBank,McountryCentralBank,
          ...
          for country in McountryConsumer:
              ...
              # Size-distribution statistics (inline — no separate file)
              from sizeDistribution import SizeDistribution as _SD
              self.DcountryCollectData[country].extend(
                  _SD.get_stats(country, McountryFirm, McountryBank))
              ...
              if Y>0:
                 CAOnY=CA/float(Y)
              # CA == PrivateSaving + PublicSaving check suppressed — always
              # pass  # original: print('stop', stop) — always NameError, suppressed = NameError in original code (silent). 
              # if  self.McountryY[country]>0:
              #     if CA<PrivateSaving+PublicSaving-0.001 ...:
      ...
      def WriteInitial(self):
          ...
          L=['run','time','country','Yprod','Export','Import','nFirm', 'avWage','nOccupied',\
             ...
             'loanReceivedRegional','loanReceivedGlobal',\
             'loanDemandedRegional','loanDemandedGlobal',\
             'CA','PrivateSaving',\
             'diffCreditFlow','diffBondFlow']
          # Size-distribution columns (firms and banks, by type, per country)
          from sizeDistribution import SizeDistribution as _SD
          L += _SD.column_names()
          ...
          if  self.printAgent=='yes':
              ...
              L=['run', 'ide','time','country', 'phi','profit','p','w','l','x_prod',\
               'xE','x_sold','inventory', 'workForceNumberDesired','A','revenue','thetaType'] 
              c=open(nameCollectFirm,'w',newline='')  
              writer = csv.writer(c)
              writer.writerow(L)
              c.close()
              nameCollectBank=self.folder+'/'+self.name+'Bank.csv' 
              L=['run', 'ide','time','country', 'A','profit','Bonds','Loan','Deposit']
              c=open(nameCollectBank,'w',newline='')  
              writer = csv.writer(c)
              writer.writerow(L)
              c.close()

      def checkNetWorth(self,McountryConsumer,McountryFirm,McountryBank,McountryCentralBank,McountryEtat,DpastBondExit,McountryFund=None):
          for country in McountryConsumer:
              netWorthConsumer=0
              netWorthBank=0
              netWorthFirm=0
              ...
              try:
                  if netWorth/float(netWorthConsumer)>0.0001 or netWorth/float(netWorthConsumer)<-0.0001\
                     ...
                     pass  # original: pass  # original: print('stop', stop) — always NameError, suppressed — always NameError, never enforced
              except ZeroDivisionError:
                  pass
              McountryCentralBank[country].checkNetWorth() 
              McountryCentralBank[country].checkReserves(McountryBank,McountryFund) 
