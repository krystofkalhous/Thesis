...
class Etat:
      def __init__(self,country,taxRate,delta,G,rBonds,\
          ...
          # optional debt-stock fiscal rule (default OFF -> byte-identical baseline)
          self._fiscalRuleOn=_ef('FISCAL_RULE_ON',0.0)>=0.5
          self._fiscalDebtTarget=_ef('FISCAL_DEBT_TGT',2.0)
          self._fiscalTaxMax=_ef('FISCAL_TAXMAX',0.65)
      ...
      def governmentPlannedExpenditure(self,McountryConsumer,McountryFirm,McountryCentralBank,\
          ...
          if self.startingTime>=self.endTransitionTime: 
             ...
             # --- optional debt-stock fiscal rule: bang-bang max austerity ---
             if self._fiscalRuleOn and McountryY[self.country]>0:
                _dr=self.pastBonds/float(McountryY[self.country])
                if _dr>self._fiscalDebtTarget:
                   self.followingTaxRate=self._fiscalTaxMax      # tax above normal cap
                   self.realG=self.gMin*McountryY[self.country]  # spending at floor
             self.G=self.realG  
