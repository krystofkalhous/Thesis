# centralbank.py
import os
# --- single-country isolation: when on, each country's policy rate responds to its OWN inflation
#     (instead of the union average). Off = original union-wide Taylor rule (byte-identical).
_TAYLOR_PER_COUNTRY=os.environ.get("TAYLOR_PER_COUNTRY","0").lower() in ("1","true","yes","on")
...
class CentralBankUnion:
      ...
      def checknetWorth(self):
          if self.DepositCentralBankUnion>self.LoanCentralBankUnion+0.00001 or\
             ...
             raise AssertionError("SFC invariant violated: centralBankUnion.py:53 in checknetWorth()")
      ...
      def taylorRule1(self,McountryInflation,McountryUnemployement,McountryBank,McountryCentralBank,t,McountryConsumer,McountryFirm):
         if t>self.endTransitionTime:
          ...
          rcB=self.rBar*(1-self.csi)+self.csi*self.rDiscount+(1-self.csi)*self.csiDP*(inflation-self.inflationTarget)
          self.rDiscount=max(0,rcB)
          if _TAYLOR_PER_COUNTRY and not hasattr(self,'McountryRDiscount'):
              self.McountryRDiscount={c:self.rDiscount for c in McountryBank}
          for country in McountryBank:
              if _TAYLOR_PER_COUNTRY:
                  _rPrev=self.McountryRDiscount.get(country,self.rDiscount)
                  _rcBc=self.rBar*(1-self.csi)+self.csi*_rPrev+(1-self.csi)*self.csiDP*(McountryInflation[country]-self.inflationTarget)
                  _rC=max(0,_rcBc)
                  self.McountryRDiscount[country]=_rC
              else:
                  _rC=self.rDiscount
              McountryCentralBank[country].rDiscount=_rC
              rDeposit=self.zeta*_rC
              McountryCentralBank[country].rDeposit=rDeposit 
              for bank in McountryBank[country]:
                  McountryBank[country][bank].rDiscount=_rC
                  McountryBank[country][bank].rDeposit=rDeposit
