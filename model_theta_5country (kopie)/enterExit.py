...
# ---- theta extension: latent-type assignment at firm birth; default OFF ----
_THETA_ON=os.environ.get("THETA_ON","0").lower() in ("1","true","yes","on")
_THETA_SPREAD=float(os.environ.get("THETA_SPREAD","0.5"))   # sigma of log latent type (0 => no heterogeneity)
          ...
          for country in McountryConsumer: 
              ...
              while True:
                  ...
                  if newAgentKind in ('firmNotTradable', 'firmTradable'):
                      ...
                      if _THETA_ON and _THETA_SPREAD>0:
                          # lognormal, mean 1 (exp(eta - sigma^2/2)): turning theta on
                          # adds DISPERSION only, no mechanical level shift.
                          _eta=random.gauss(0.0,_THETA_SPREAD)
                          newfirm.thetaType=math.exp(_eta-0.5*_THETA_SPREAD*_THETA_SPREAD)
