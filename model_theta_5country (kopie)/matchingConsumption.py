...
import math
import os
# --- single-country isolation: when on, tradable consumers buy only same-country tradables
#     (home bias = 1). Preserves propTradable (consumption structure) but cuts cross-border trade.
_HOME_BIAS=os.environ.get("HOME_BIAS","0").lower() in ("1","true","yes","on")
...
class MatchingConsumption:
      ...
      def consumptionConsumer5(self,McountryConsumer,McountryFirm,t,McountryEtat,McountryBank,McountryCentralBank,McountryAvPrice,avPriceGlobalTradable):
             ...
             for Lconsumer in self.Lconsuming:
                 ...
                 if lenSale>0: 
                    if _HOME_BIAS:
                        LlenSale=[i for i in range(lenSale) if self.Lsale[i][3]==country]
                    else:
                        LlenSale=range(lenSale)
                    length=min(length,len(LlenSale))
                    if length>0:
                        LchoiceGood=random.sample(list(LlenSale),length)
                 ...
                 if len(LconsumptionU)>0:
                    ...
                    while j<lj:
                          ...
                          if consumption<=0:
                             lj=0
                             if consumption<-0.01:
                                raise AssertionError("SFC invariant violated: matchingConsumption.py:150 in consumptionConsumer5()")
                          if consumption>0:
                             j=j+1
                    shift=0
                 ...
                 if len(LconsumptionU)>0:
                    ...
                    while j<lj:
                          ...
                          if consumption<-0.0001:
                             raise AssertionError("SFC invariant violated: matchingConsumption.py:221 in consumptionConsumer5NotTradable()")
                          McountryConsumer[country][consumer].LConsumptionEff.append([p,quantityConsumed,firmide,firmcountry])
                          McountryConsumer[country][consumer].Expenditure=McountryConsumer[country][consumer].Expenditure+quantityConsumed*p
                          ...
                          if consumption<=0:
                             lj=0
                             if consumption<-0.0001:
                                raise AssertionError("SFC invariant violated: matchingConsumption.py:245 in consumptionConsumer5NotTradable()")
                          if consumption>0:
                             j=j+1
                    shift=0
                    McountryConsumer[country][consumer].paying(totalPayment,McountryBank,McountryCentralBank)
