...
class enterExit:
      ...
      def minMaxAgents(self,McountryFirm,McountryBank,country,McountryAvPrice):
          ...
          if len(LbankSize)>0:
             ...
             self.maxBankSize=max(LbankSize) ### This I will need to modify -> bank_Small is smaller!!!
          ...
          if len(LfirmSizeNotTradable)>0:
             ...
             ### ----
             # Modify maxFirmSizeNotTradable, to sample entrants only from 1st (15%) quartil of distribution
             s = sorted(LfirmSizeNotTradable)
             rank = math.ceil(0.15 * len(s)) 
             self.maxFirmSizeNotTradable = s[rank]
             ### ---
      ...
      def whichSize_modyfied_for_bank_types(self,kind):
          if kind=='firmNotTradable':
             ...
             phi=self.whichPhi('firmNotTradable')
             wage=self.whichWage('firmNotTradable')
             # try to sample from quartile
             size=max(self.initialA,wage,random.uniform(self.minFirmSizeNotTradable,self.maxFirmSizeNotTradable)) ### I modified what is considered max here, sampling up to quartil 25%
             eX=self.whichEx('firmNotTradable',phi,price,wage)
             Lkind=[kind,size,phi,price,wage,eX]
          elif kind=='firmTradable':
             price=self.whichPrice('firmTradable')
             phi=self.whichPhi('firmTradable')
             wage=self.whichWage('firmTradable')
             size=max(self.initialA,wage,random.uniform(self.minFirmSizeTradable,self.maxFirmSizeTradable))
          ...
          elif kind in ('bank_Small', 'bank_Big'): ### At this stage this affects only the loan pricing / approval behaviour of the bank
             ### size=random.uniform(self.minBankSizePast,self.maxBankSizePast) ### This seems redundant !!!
             sizeMin=max(self.sigma*self.medianFirmSizeNotTradable,4*self.medianFirmSizeTradable,self.minBankSizePast) 
             size=random.uniform(sizeMin,self.maxBankSizePast)
             Lkind=[kind,size]
      ...
      ### ----------------------------------------------------------------
      ### MY CODE
      def which_spatial_Position(self, agent_type):
         if agent_type in ('firmNotTradable', 'firmTradable'):
            spatial_position = self.rng.random()

         elif agent_type in ('bank_Small', 'bank_Big'):
            spatial_position = self.rng.uniform(0.0, 1.0) ### Experiment
            # spatial_position = self.rng.gauss(mu=0.0, sigma=0.2) ### Experiment
            # spatial_position = self.rng.random() ### Originally
      ...
      def fork(self, country, McountryBank, fork_input: str):
         ...
         """This function will apply aditional criterion implemented in function which_Bank_Size() on the output of the 
            ...
            """
         if fork_input in ('firmTradable', 'firmNotTradable'):
            return fork_input
         
         elif fork_input == 'bank':
            return self.which_Bank_Size(country, McountryBank, fork_input)
         
         else:
            raise ValueError(f"Unexpected kind before fork: {fork_input!r}")
...
### Here I am adding very simple 'stochastic' rule, that I will most likely modify later
         ...
         if random_value >= 0.2: ### Modify,now always create big bank. originally big with prob 20% and small with prob 80%
            newAgent = 'bank_Small'
...
### !!! ###
      ...
      def enter(self,McountryConsumer,McountryFirm,time,McountryBank,McountryCentralBank,McountryAvPrice,McountryEtat):
          ...
          for country in McountryConsumer: 
              ...
              ### ----------------------------------------------------------------
              ### MY CODE
              newAgentKind = self.fork(country, McountryBank, newAgentKind)
              Lkind=self.whichSize_modyfied_for_bank_types(newAgentKind) ### changed wchichSize to whichSize_modyfied_for_bank_types()
              ### ----------------------------------------------------------------
                    ...
                    if sumInv>=newAgentSize:
                       if newAgentKind=='firmNotTradable' or newAgentKind=='firmTradable':
                          if newAgentKind=='firmNotTradable':
                             ...
                             ### ----------------------------------------------------------------
                             ### MY MODIFICATION: adding spatial position to the Firm "instance generator"

                             spatial_position = self.which_spatial_Position(newAgentKind)
                        ...
                        ### ----------------------------------------------------------------
                        ### MY MODIFICATION: adding spatial position to the Firm "instance generator"
                       if newAgentKind=='bank_Small' or newAgentKind=='bank_Big':
                          
                          # added here
                          bank_type = newAgentKind
                          spatial_position = self.which_spatial_Position(newAgentKind)
                          ...
                          newbank=Bank(bankide,country,newbankA,self.Lcountry,self.Fcost,self.folder,self.name,\
                                    ...
                                    bank_type, spatial_position) # added: bank_type and spatial_position
                  ...
                  if McountryFirm[country][firm].loanReceived>0.0:  
                     ...
                     for bank in McountryFirm[country][firm].Mloan: 
                         ...
                         if McountryBank[countryBank][ideBank].ide==ideBank:
                            ...
                            if losses<-0.001: 
                               pass
                               # print('stop', stop) ### Modify, not raise error !!! -> change later
