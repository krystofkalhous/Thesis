...
class Parameter:
      def __init__(self):                         
          self.name= 'muxSnCo1upsilon20.85polnnPolVar0.512' ### orginally: 'muxSnCo5upsilon20.7polModPolVar0.512' #1.625'#2.876'#2.231#1.625'#1.053#0.512'
          ...
          self.upsilon2=0.85#(upsilon2)   ### 0.85 -> 0.87->0.865 -> try (fix after excess firms) 0.825 ->(too much try:) 0.850
          self.phi=1.0 # (phi_0)
          ...
          self.Fcost=1.5# (F) ### raise from 1.0 ->1.5 (model was overcrouded w firms)
          self.minMarkUp=0.0#0.0# (minimum mark-up)
          self.theta=0.10 ### from 0.20 -> 0.10
          self.jobDuration=self.ncycle #self.ncycle#40 ### previous 2 -> 4 ### this is inactive !!!if active 2 (smooths)-> smoother?: 4 -> using 2, 4 does not help much ### this is the reason why so jumpy 2 -> self.ncycle (now it doesnt work with 2)
          #consumers         
          ...
          self.ls=1.0 #(l^S) 
          self.wBar=0.10 #(w bar) ### try 0.1 ->0.12 (doesnt work) ->010
          self.w0=1.0 #(w_0)          
          #bank
