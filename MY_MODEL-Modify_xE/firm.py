...
class Firm:
      ...
      def learning(self):   
          if self.closing=='no':
             ...
             self.mind.alphaParameterSmooth17(self.phi,self.w,self.inventory,self.pastInventory,\
                                               self.price,self.productionEffective,self.xSold)
             ###
             self.pastPrice=self.price
      ...
      def wageOffered1(self,McountryUnemployement,McountryPastUnemployement,McountryYL,McountryPastYL,t,McountryConsumer):
          ...
          if self.w<self.pastWage*(1-0.03):
             self.w=self.pastWage*(1-0.03)
          # if self.ide=='F0n0': ### -> To avoid printing constantly
          #   print()
      ...
      def wageOffered(self,McountryUnemployement,McountryPastUnemployement,McountryYL,McountryPastYL,t,McountryConsumer):
          ...
          self.pastWage=self.w
          direction='stay'
          firmUpsilon=0.90#1/2.5#0.25 ### -> try 0.90
...
#### -------
      def wageOffered_custom1(self, McountryUnemployement, McountryPastUnemployement,McountryYL, McountryPastYL, t, McountryConsumer): # One country model calibration
         ...
         # --- Original adaptive wage rule ---
         # If the firm wanted more workers than it employed, it may raise wages.
         if nWorkerDesired > self.l:
            if a <= firmUpsilon * math.exp(-u * self.upsilon):
               direction = 'plus'
            else:
         ...
         # If the firm had enough workers, it may reduce wages.
         if nWorkerDesired <= self.l:
            if a > firmUpsilon * math.exp(-u * self.upsilon):
               direction = 'minus'
            else:
         ...
         # --- New: small productivity-sharing component ---
         # This is intentionally weak. Do not start with 0.30. ### -> tweak later?
         prodShare = 0.05 # Past 0.10 does ont work well, prodShare = 0.05 (helps a little _A4)

         if self.gPhi > 0:
            self.w = self.w * (1 + prodShare * self.gPhi)
      ...
      def wageOffered_custom2(self, McountryUnemployement, McountryPastUnemployement,McountryYL, McountryPastYL, t, McountryConsumer): # One country model calibration
         ...
         direction = 'stay'
         firmUpsilon = 0.90 ### too hot labour market 1.0 -> 0.90 -> levels good but too jumpy, try: 0.925 ->0.90
         ...
         prodGrowth = 0.0
         if McountryPastYL[self.country] > 0:
            prodGrowth = (McountryYL[self.country] - McountryPastYL[self.country]) / float(McountryPastYL[self.country])

         if prodGrowth > 0:
            self.w = self.w * (1 + prodShare * prodGrowth)
